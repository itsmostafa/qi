...
- **Date filters.** `--since`, `--until` and `--sort date` on `search` and `query`, filtered in SQL against `documents.doc_timestamp`. Documents indexed before migration 006 have a NULL timestamp and are excluded until reindexed, which is what `qi index --force` is for.
  ...
  **Status (2026-09-04): superseded.** A NULL timestamp was not confined to pre-006 rows — `doc_timestamp` was only ever populated from frontmatter `timestamp`/`date`, so every document without dated frontmatter stayed NULL, and NULL satisfies neither predicate. `--since`/`--until` therefore returned nothing on corpora that don't use dated frontmatter, which is most of them. `documentDate` (`internal/indexer`) now falls back to the file's mtime, `created` joins the frontmatter aliases, and a plain `qi index` backfills NULL rows without re-chunking or re-embedding. `--force` is no longer the backfill mechanism.
