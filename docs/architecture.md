...
1. **Parse** — files are broken into sections by structure. Markdown, MDX, reStructuredText and AsciiDoc split at headings; plaintext files are treated as a single section. Parsers are registered by extension, so adding a new format is one file and one function call — nothing else changes.
