...
- **Pull hooks, not a watcher**: `qi hook install` writes `post-merge`/`post-rewrite` hooks that run `qi index --changed-since ORIG_HEAD` in the background. The extension check lives in Go (`docsChangedSince`, `cmd/index.go`), not in the hook script, so the hook never goes stale when a collection's `extensions` change; if git cannot answer, the run indexes everything.
- **Auto-embed on index**: When an embedder is configured, chunks are embedded immediately after indexing without a separate step. Finding what to embed (`pendingChunks`) is pure SQL — missing rows, dimension, fingerprint and `length(vector)` — so a no-op run never reads a vector (4.7s → 1.85s at 424k chunks). Finite/non-zero values are enforced at write time only; a vector corrupted outside qi is skipped by search and reported by `qi doctor`, not repaired.
